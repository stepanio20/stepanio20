"""/sell — consignment lead form (one-message FSM)."""
...
async def cmd_sell(msg: Message, db: Database, state: FSMContext):
    ...
    await msg.answer(t("sell_pitch", lang))
...
async def cb_sell(cb: CallbackQuery, db: Database, state: FSMContext):
    ...
    await state.set_state(SellForm.waiting_description)
    await cb.message.answer(t("sell_pitch", lang))
    await cb.answer()


@router.message(SellForm.waiting_description)
    ...
    for admin_id in cfg.admin_ids:
        try:
            await msg.bot.send_message(
                admin_id,
                f"💼 <b>Новая заявка на продажу #{lead_id}</b>\n"
                f"От: {contact}\n\n{description[:3500]}",
            )
        except Exception:
            pass
