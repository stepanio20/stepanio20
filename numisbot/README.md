...
| 🏷 `/estimate` | Оценка монеты по реальным проходам (диапазон, медиана, аналоги) | квоты 1/5/15/30 в мес |
| 🧺 `/portfolio` | Портфель коллекции — переоценка по свежим торгам при каждом открытии | слоты 5/100/500 |
| 📦 Aftersale-фид | Ежедневно: непроданные лоты по вашим темам — можно спросить у Katz | Sniper+ / Dealer |
| 🎁 `/invite` | +30 дней Pro за активированного друга (кап 6/год) | всем |
| 📬 Дайджест и анонсы | Понедельничный топ по интересам + «новый аукцион» (`/digest` — выкл) | всем |
| ⭐ `/pro` | Stars-подписки с автопродлением: Pro 299⭐ / Sniper+ 749⭐ / Dealer 1900⭐; `/paysupport`, админ-`/refund` | — |
| 📊 `/stats`, `/channelpost` | Метрики и готовый пост топ-лотов для канала (админ) | admin |

**Языки:** RU · EN · DE · CS (полная локализация, языки аудитории Katz).

**Безопасность:** витрина премодерируемая (листинги видны только после ✅ админа),
rate-limit 25 апдейтов/мин/юзер + только личные чаты, семафор на внешние поиски,
кулдаун и авто-бан публикаций после 3 отклонений, обязательный @username для продавца,
идемпотентные платежи, `/forgetme` (полное удаление данных, GDPR).
